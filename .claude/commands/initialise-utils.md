...
- Dotnet test should not use the --no-build flag, it should rebuild if required, it should use similar flags to build, e.g. dotnet test src/GraphlessDB.sln --no-incremental -p:UseSharedCompilation=false -p:UseRazorBuildServer=false /nodeReuse:false --verbosity quiet
...
- Filter files to only include those from the project being tested (e.g., only include files from src/GraphlessDB.DynamoDB/ when testing GraphlessDB.DynamoDB)
...
Verify the correctness of the output by comparing the output for file "./src/GraphlessDB.DynamoDB/DynamoDB.Transactions.Storage/VersionedItemStore.cs" against another coverage report which can be run using "./src/build.sh" and looking at the html report at "./src/bin/test/results/codecoverage/GraphlessDB.DynamoDB_VersionedItemStore.html"

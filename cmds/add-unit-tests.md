...
Run get-least-covered-file.sh to get the name of a file which requires unit testing to increase its code coverage.  Then determine and implement any missing unit tests for that file. You should iterate using get-file-coverage.sh until the  nonCovered lines has reached 0.  Once complete run save-solution-coverage.sh.

...
{
    [TestClass]
    public sealed class CommittedIsolatedGetItemServiceTests
    {
        ...
        private sealed class MockTransactionStore : ITransactionStore
        {
            ...
            public Task<Transaction> GetAsync(TransactionId id, bool forceFetch, CancellationToken cancellationToken)
            {
                return _getAsync?.Invoke(id, forceFetch, cancellationToken)
                    ?? Task.FromResult(new Transaction(id.Id, TransactionState.Committed, 1, DateTime.UtcNow, ImmutableList<RequestRecord>.Empty));
            }
        ...
        private sealed class MockVersionedItemStore : IVersionedItemStore
        {
            ...
            public Task<ImmutableList<ItemRecord>> GetItemsToBackupAsync(AmazonDynamoDBRequest request, CancellationToken cancellationToken) => throw new NotImplementedException();
            public Task<ImmutableDictionary<ItemKey, ItemTransactionState>> AcquireLocksAsync(Transaction transaction, AmazonDynamoDBRequest request, CancellationToken cancellationToken) => throw new NotImplementedException();
            ...
            public Task ReleaseLocksAsync(Transaction transaction, bool rollback, ImmutableDictionary<ItemKey, ItemRecord> rollbackImagesByKey, CancellationToken cancellationToken) => throw new NotImplementedException();
            public Task ReleaseLocksAsync(TransactionId id, TransactionId owningTransactionId, ImmutableList<ItemKey> itemKeys, bool rollback, ImmutableDictionary<ItemKey, ItemTransactionState> itemTransactionStatesByKey, ImmutableDictionary<ItemKey, ItemRecord> rollbackImagesByKey, CancellationToken cancellationToken) => throw new NotImplementedException();
        }

        private sealed class MockItemImageStore : IItemImageStore
        {
            private readonly Func<TransactionVersion, CancellationToken, Task<ImmutableList<ItemRecord>>>? _getItemImagesAsync;

            public MockItemImageStore(
                Func<TransactionVersion, CancellationToken, Task<ImmutableList<ItemRecord>>>? getItemImagesAsync = null)
            {
                _getItemImagesAsync = getItemImagesAsync;
            }

            public Task<ImmutableList<ItemRecord>> GetItemImagesAsync(TransactionVersion transactionVersion, CancellationToken cancellationToken)
            {
                return _getItemImagesAsync?.Invoke(transactionVersion, cancellationToken)
                    ?? Task.FromResult(ImmutableList<ItemRecord>.Empty);
            }

            public Dictionary<string, AttributeValue> GetKey(TransactionVersion transactionVersion) => throw new NotImplementedException();
            public Task<ImmutableList<TransactionVersion>> GetTransactionVersions(TransactionId id, CancellationToken cancellationToken) => throw new NotImplementedException();
            public Task AddItemImagesAsync(TransactionVersion transactionVersion, ImmutableList<ItemRecord> items, CancellationToken cancellationToken) => throw new NotImplementedException();
            public Task DeleteItemImagesAsync(TransactionVersion transactionVersion, CancellationToken cancellationToken) => throw new NotImplementedException();
        }
        ...
        private sealed class MockRequestService : IRequestService
        {
            ...
            public Task<ImmutableList<ItemRequestDetail>> GetItemRequestDetailsAsync(AmazonDynamoDBRequest request, CancellationToken cancellationToken) => throw new NotImplementedException();
        }
        ...
        public async Task GetItemAsyncReturnsItemWithoutTransaction()
        {
            ...
            var service = new CommittedIsolatedGetItemService(
                mockClient,
        ...
        [TestMethod]
        public async Task BatchGetItemAsyncReturnsItemsWithoutTransactions()
        {
        ...
        [TestMethod]
        public async Task TransactGetItemsAsyncReturnsItemsWithoutTransactions()
        {
            var key1 = CreateKey("test-id-1");
            var item1 = CreateItem("test-id-1", "value-1");
        ...
        [TestMethod]
        public async Task GetItemAsyncWithCommittedTransactionReturnsItem()
        {
            var key = CreateKey("test-id");
            var item = CreateItem("test-id", "test-value");
            ...
            var mockTransactionStore = new MockTransactionStore(
                getAsync: (id, forceFetch, ct) =>
                    Task.FromResult(new Transaction(id.Id, TransactionState.Committed, 1, DateTime.UtcNow, ImmutableList<RequestRecord>.Empty)));

            var mockVersionedItemStore = new MockVersionedItemStore(
                getItemRecordAndTransactionState: (i) => new Tuple<Dictionary<string, AttributeValue>, TransactionStateValue>(
                    i,
                    new TransactionStateValue(true, transactionId, null, false, true)));
        ...
        [TestMethod]
        public async Task GetItemAsyncWithCommittingTransactionReturnsItem()
        {
            var key = CreateKey("test-id");
            var item = CreateItem("test-id", "test-value");
            var transactionId = "tx-123";
            ...
            var mockTransactionStore = new MockTransactionStore(
                getAsync: (id, forceFetch, ct) =>
                    Task.FromResult(new Transaction(id.Id, TransactionState.Committing, 1, DateTime.UtcNow, ImmutableList<RequestRecord>.Empty)));
        ...
        [TestMethod]
        public async Task GetItemAsyncWithTransientItemReturnsEmptyItem()
        {
            var key = CreateKey("test-id");
            var item = CreateItem("test-id", "test-value");
            var transactionId = "tx-123";
            ...
            var mockTransactionStore = new MockTransactionStore(
                getAsync: (id, forceFetch, ct) =>
                    Task.FromResult(new Transaction(id.Id, TransactionState.Active, 1, DateTime.UtcNow, ImmutableList<RequestRecord>.Empty)));

            var mockVersionedItemStore = new MockVersionedItemStore(
                getItemRecordAndTransactionState: (i) => new Tuple<Dictionary<string, AttributeValue>, TransactionStateValue>(
                    i,
                    new TransactionStateValue(true, transactionId, null, true, false)));
        ...
        [TestMethod]
        public async Task GetItemAsyncWithActiveTransactionAndDeleteRequestReturnsItemRecord()
        {
            var key = CreateKey("test-id");
            var item = CreateItem("test-id", "test-value");
            var transactionId = "tx-123";
            ...
            var mockRequestService = new MockRequestService(
                ...
                {
                    var lockedAction = new LockedItemRequestAction(itemKey, 1, RequestAction.Delete);
                    return Task.FromResult(ImmutableList.Create(lockedAction));
                });
        ...
        [TestMethod]
        public async Task GetItemAsyncWithActiveTransactionAndPutRequestReturnsItemImage()
        {
            var key = CreateKey("test-id");
            var item = CreateItem("test-id", "test-value");
            var transactionId = "tx-123";
            ...
            var itemImageAttributes = new Dictionary<string, ImmutableAttributeValue>
            {
                { "Id", ImmutableAttributeValue.Create(AttributeValueFactory.CreateS("test-id")) },
                { "Value", ImmutableAttributeValue.Create(AttributeValueFactory.CreateS("old-value")) }
            };
            var itemImage = new ItemRecord(itemKey, itemImageAttributes.ToImmutableDictionary());
            ...
            var mockItemImageStore = new MockItemImageStore(
                getItemImagesAsync: (tv, ct) =>
                {
                    return Task.FromResult(ImmutableList.Create(itemImage));
                });
            ...
            Assert.IsNotNull(response.Item);
            Assert.AreEqual(2, response.Item.Count);
            Assert.AreEqual("test-id", response.Item["Id"].S);
            Assert.AreEqual("old-value", response.Item["Value"].S);
        }

        [TestMethod]
        [ExpectedException(typeof(TransactionException))]
        public async Task GetItemAsyncWithAppliedButActiveTransactionThrowsException()
        {
            var key = CreateKey("test-id");
            var item = CreateItem("test-id", "test-value");
            var transactionId = "tx-123";
            ...
            var mockRequestService = new MockRequestService(
                getItemRequestActionsAsync: (tx, ct) => Task.FromResult(ImmutableList<LockedItemRequestAction>.Empty.Add(
                    new LockedItemRequestAction(itemKey, 1, RequestAction.Put))));
        ...
        [TestMethod]
        public async Task GetItemAsyncWithUnappliedTransactionReturnsItem()
        {
            var key = CreateKey("test-id");
            var item = CreateItem("test-id", "test-value");
            var transactionId = "tx-123";
            ...
            var mockVersionedItemStore = new MockVersionedItemStore(
                getItemRecordAndTransactionState: (i) => new Tuple<Dictionary<string, AttributeValue>, TransactionStateValue>(
                    item,
                    new TransactionStateValue(true, transactionId, null, false, false)));

            var mockRequestService = new MockRequestService(
                getItemRequestActionsAsync: (tx, ct) => Task.FromResult(ImmutableList<LockedItemRequestAction>.Empty.Add(
                    new LockedItemRequestAction(itemKey, 1, RequestAction.Put))));

            var itemImages = ImmutableList<ItemRecord>.Empty.Add(
                new ItemRecord(itemKey, ImmutableDictionary<string, ImmutableAttributeValue>.Empty
                    .Add("Id", new ImmutableAttributeValue("test-id", null, null, null, null, null, null))
                    .Add("Value", new ImmutableAttributeValue("test-value", null, null, null, null, null, null))));

            var mockItemImageStore = new MockItemImageStore(
                getItemImagesAsync: (tv, ct) => Task.FromResult(itemImages));
        ...
        [TestMethod]
        public async Task GetItemAsyncWithAppliedItemButNoTransactionIdReturnsItem()
        {
            var key = CreateKey("test-id");
            var item = CreateItem("test-id", "test-value");
            ...
            var mockVersionedItemStore = new MockVersionedItemStore(
                getItemRecordAndTransactionState: (i) => new Tuple<Dictionary<string, AttributeValue>, TransactionStateValue>(
                    item,
                    new TransactionStateValue(true, null, null, false, true)));

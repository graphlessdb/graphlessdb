...
namespace GraphlessDB.DynamoDB.Transactions.Tests
{
    ...
    public sealed class VersionedItemStoreTests
    {
        ...
        [TestMethod]
        public async Task AcquireLocksAsyncShouldThrowNotSupportedExceptionForUnsupportedRequestActionInGuessExists()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();

            var requestService = new MockRequestService(
                getItemRequestActionsAsync: (tx, ct) =>
                {
                    return Task.FromResult(ImmutableList<LockedItemRequestAction>.Empty.Add(
                        new LockedItemRequestAction(itemKey, 0, (RequestAction)999)));
                },
                getItemRequestDetailsAsync: (req, ct) =>
                {
                    return Task.FromResult(ImmutableList<ItemRequestDetail>.Empty.Add(
                        new ItemRequestDetail(itemKey, (RequestAction)999, string.Empty,
                            ImmutableDictionary<string, string>.Empty, ImmutableDictionary<string, ImmutableAttributeValue>.Empty)));
                });
        ...
        [TestMethod]
        public async Task ApplyRequestAsyncShouldHandleTransactGetItemsRequestWithDeleteRequestAction()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
            ...
            Assert.IsNotNull(result);
            var getResult = result as TransactGetItemsResponse;
            Assert.IsNotNull(getResult);
            Assert.AreEqual(1, getResult.Responses.Count);
            Assert.AreEqual(0, getResult.Responses[0].Item.Count);
        }

        [TestMethod]
        public async Task ApplyRequestAsyncShouldHandleTransactGetItemsRequestWithGetRequestActionAndIsTransient()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
        ...
        [TestMethod]
        public async Task ApplyRequestAsyncShouldHandleTransactGetItemsRequestWithProjectionExpressionMapping()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
            ...
            var itemRecord = new ItemRecord(
                ...
                new Dictionary<string, ImmutableAttributeValue>
                {
                    { "Id", ImmutableAttributeValue.Create(AttributeValueFactory.CreateS("TestId")) },
                    { "Name", ImmutableAttributeValue.Create(AttributeValueFactory.CreateS("TestName")) },
                    { "Age", ImmutableAttributeValue.Create(AttributeValueFactory.CreateN("30")) }
                }.ToImmutableDictionary());
            ...
            Assert.IsNotNull(getResult);
            Assert.AreEqual(1, getResult.Responses.Count);
            Assert.AreEqual(2, getResult.Responses[0].Item.Count);
            Assert.IsTrue(getResult.Responses[0].Item.ContainsKey("Name"));
            Assert.IsTrue(getResult.Responses[0].Item.ContainsKey("Age"));
            Assert.IsFalse(getResult.Responses[0].Item.ContainsKey("Id"));
        }

        [TestMethod]
        public void GetItemRecordAndTransactionStateShouldHandleNullAttributeValue()
        {
            var store = CreateVersionedItemStore();
            var item = new Dictionary<string, AttributeValue>
            {
                { "Id", AttributeValueFactory.CreateS("TestId") },
                { "NullField", AttributeValueFactory.CreateNULL(true) }
            };

            var (itemRecord, transactionState) = store.GetItemRecordAndTransactionState(item);

            Assert.IsNotNull(itemRecord);
            Assert.AreEqual(2, itemRecord.Count);
            Assert.IsTrue(itemRecord.ContainsKey("NullField"));
        }

        [TestMethod]
        public async Task AcquireLocksAsyncShouldHandleGetCorrectedItemTransactionStatesWithoutFailedItemMatch()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            ...
            var requestService = new MockRequestService(
                ...
                {
                    return Task.FromResult(ImmutableList<ItemRequestDetail>.Empty.Add(
                        new ItemRequestDetail(itemKey, RequestAction.Put, string.Empty,
                            ImmutableDictionary<string, string>.Empty, ImmutableDictionary<string, ImmutableAttributeValue>.Empty)));
                });
        ...
        [TestMethod]
        public async Task AcquireLocksAsyncShouldHandleGetConflictedItemsWithNonConditionalCheckFailedCode()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
        ...
        [TestMethod]
        public async Task AcquireLocksAsyncThrowsTransactionConflictedExceptionWhenConflictedItemsNotEmpty()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
                ...
                transactWriteItemsAsync: (req, ct) =>
                {
                    ...
                    ex.CancellationReasons = new List<CancellationReason>
                    {
                        new CancellationReason
                        {
                            ...
                            {
                                { "Id", AttributeValueFactory.CreateS("TestId") },
                                { ItemAttributeName.TXID.Value, AttributeValueFactory.CreateS("different-tx-id") },
                                { ItemAttributeName.DATE.Value, AttributeValueFactory.CreateN(DateTime.UtcNow.Ticks.ToString(CultureInfo.InvariantCulture)) },
                            }
                        }
                    };
        ...
        [TestMethod]
        public async Task ApplyRequestAsyncWithUpdateItemRequestAndReturnAllOldThrowsNotSupported()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = CreateItemTransactionState(itemKey, transaction, RequestAction.Update);
            ...
            var request = new ApplyRequestRequest(
                transaction,
            ...
            await Assert.ThrowsExceptionAsync<NotSupportedException>(async () =>
            {
                await store.ApplyRequestAsync(request, CancellationToken.None);
            });
        }

        [TestMethod]
        public async Task ApplyRequestAsyncWithUpdateItemRequestAndReturnAllOldForTransientItemReturnsEmpty()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
            ...
            var result = await store.ApplyRequestAsync(request, CancellationToken.None) as UpdateItemResponse;
        ...
        [TestMethod]
        public async Task ApplyRequestAsyncWithUpdateItemNotAppliedAndReturnAllOldForTransientItemReturnsEmpty()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
            ...
            var amazonDynamoDB = new MockAmazonDynamoDB(
                updateItemAsync: (req, _) => Task.FromResult(new UpdateItemResponse()));
        ...
        [TestMethod]
        public async Task ApplyRequestAsyncWithUpdateItemNotAppliedAndReturnAllOldForNonTransientItemReturnsBackupItem()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
            ...
            var itemRecord = new ItemRecord(
                ...
                ImmutableDictionary<string, ImmutableAttributeValue>.Empty
                    .Add("Id", ImmutableAttributeValue.Create(AttributeValueFactory.CreateS("TestId")))
                    .Add("Name", ImmutableAttributeValue.Create(AttributeValueFactory.CreateS("OldName"))));
            ...
            Assert.IsTrue(result.Attributes.ContainsKey("Name"));
        }

        [TestMethod]
        public async Task ApplyRequestAsyncWithUpdateItemRequestAndUnsupportedReturnValueThrowsNotSupported()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = CreateItemTransactionState(itemKey, transaction, RequestAction.Update);
        ...
        [TestMethod]
        public async Task ApplyRequestAsyncWithTransactWriteItemsRequestThrowsWhenUnlockedItemsExist()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
            ...
            var transactionStore = new MockTransactionStore(
                getAsync: (id, forceFetch, ct) => Task.FromResult(transaction));
            ...
            var store = CreateVersionedItemStore(requestService: requestService, transactionStore: transactionStore);
        ...
        [TestMethod]
        public async Task ApplyRequestAsyncWithTransactWriteItemsRequestReturnsEmptyResponseWhenNoTransactItems()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = CreateItemTransactionState(itemKey, transaction, RequestAction.Delete);
            ...
            var requestService = new MockRequestService(
                ...
                {
                    return Task.FromResult(ImmutableList<ItemRequestDetail>.Empty.Add(
                        new ItemRequestDetail(itemKey, RequestAction.Delete, string.Empty,
                            ImmutableDictionary<string, string>.Empty, ImmutableDictionary<string, ImmutableAttributeValue>.Empty)));
                });
            ...
            var result = await store.ApplyRequestAsync(request, CancellationToken.None) as TransactWriteItemsResponse;

            Assert.IsNotNull(result);
        }

        [TestMethod]
        public async Task ApplyRequestAsyncWithPutItemRequestAndReturnAllOldForAppliedNonTransientItemThrowsNotSupported()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = CreateItemTransactionState(itemKey, transaction, RequestAction.Put);
        ...
        [TestMethod]
        public async Task ApplyRequestAsyncWithPutItemNotAppliedAndReturnAllOldForNonTransientItemReturnsBackupItem()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
            ...
            var amazonDynamoDB = new MockAmazonDynamoDB(
                putItemAsync: (req, _) => Task.FromResult(new PutItemResponse()));
            ...
            var result = await store.ApplyRequestAsync(request, CancellationToken.None) as PutItemResponse;
        ...
        [TestMethod]
        public async Task AcquireLocksAsyncSecondRetryWithEmptyConflictsThrowsTransactionConflictedException()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction("tx-123");
            var requestService = new MockRequestService(
        ...
        [TestMethod]
        public async Task ReleaseLocksAsyncOverloadCallsEventHookWhenProvided()
        {
            var itemKey = CreateItemKey();
            var eventCalled = false;
            var transactionServiceEvents = new MockTransactionServiceEvents
            {
                OnReleaseLockFromOtherTransactionAsync = (id, owningId, _) =>
                {
                    eventCalled = true;
                    return Task.CompletedTask;
                }
            ...
            var store = CreateVersionedItemStore(
                transactionServiceEvents: transactionServiceEvents,
                amazonDynamoDB: amazonDynamoDB);

            await store.ReleaseLocksAsync(
                new TransactionId("tx-123"),
                new TransactionId("tx-456"),
                ImmutableList.Create(itemKey),
                true,
                ImmutableDictionary<ItemKey, ItemTransactionState>.Empty.Add(itemKey, itemTransactionState),
                ImmutableDictionary<ItemKey, ItemRecord>.Empty,
        ...
        [TestMethod]
        public async Task ReleaseLocksAsyncWithEmptyTransactItemsDoesNotCallDynamoDB()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var dynamoDBCalled = false;
            ...
            var transactionStore = new MockTransactionStore(
                getAsync: (_, _, _) => Task.FromResult(transaction));

            var amazonDynamoDB = new MockAmazonDynamoDB(
                transactGetItemsAsync: (req, _) =>
                {
            ...
            Assert.IsFalse(dynamoDBCalled);
        }

        [TestMethod]
        public async Task ApplyRequestAsyncWithPutItemAppliedAndReturnAllOldForTransientItemReturnsEmptyAttributes()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
        ...
        [TestMethod]
        public async Task ApplyRequestAsyncWithPutItemAppliedAndReturnAllOldThrowsNotSupportedException()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
        ...
        [TestMethod]
        public async Task ApplyRequestAsyncWithPutItemAppliedAndReturnAllNewWithoutBackupThrowsNotSupportedException()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
        ...
        [TestMethod]
        public async Task ApplyRequestAsyncWithPutItemAppliedAndReturnAllNewWithBackupReturnsBackupItem()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
        ...
        [TestMethod]
        public async Task ApplyRequestAsyncWithPutItemAppliedAndUnsupportedReturnValueThrowsNotSupportedException()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
        ...
        [TestMethod]
        public async Task ApplyRequestAsyncWithPutItemNotAppliedAndReturnAllOldForTransientItemReturnsEmptyAttributes()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
        ...
        [TestMethod]
        public async Task ApplyRequestAsyncWithPutItemNotAppliedAndUnsupportedReturnValueThrowsNotSupportedException()
        {
            var itemKey = CreateItemKey();
            var transaction = CreateTransaction();
            var itemTransactionState = new ItemTransactionState(
